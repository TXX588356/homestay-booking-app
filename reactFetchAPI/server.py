...
def get_user_bookings(user_id):
    db = sqlite3.connect(DB)
    cursor = db.cursor()
    cursor.execute('SELECT * FROM booking WHERE userID=?', (user_id,))
    rows = cursor.fetchall()
    db.close()
...
def store():
    ...
    cursor = db.cursor()
    cursor.execute('''
        INSERT INTO booking(userID, placeID, startDate, endDate)
        VALUES(?, ?, ?, ?)
    ''', new_BookingHistory)
...
# ✅ Added route to return all bookings
@app.route('/api/bookingHistory', methods=['GET'])
def index():
    db = sqlite3.connect(DB)
    cursor = db.cursor()
